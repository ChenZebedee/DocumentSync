...
- [HBase shell 表基本语法](#hbase-shell-表基本语法)
    - [通用命令](#通用命令)
    - [添加数据](#添加数据)
    - [查询数据](#查询数据)
        - [查询某行记录](#查询某行记录)
            - [语法：](#语法)
            - [查询表t1，rowkey001中的f1下的col1的值](#查询表t1rowkey001中的f1下的col1的值)
            - [或者：](#或者)
        - [查询表t1，rowke002中的f1下的所有列值（数据多的时候 不建议使用）](#查询表t1rowke002中的f1下的所有列值数据多的时候-不建议使用)
        - [扫描表](#扫描表)
            - [语法：](#语法-1)
            - [扫描表t1的前5条数据](#扫描表t1的前5条数据)
        - [查询表中的数据行数](#查询表中的数据行数)
            - [语法：](#语法-2)
            - [查询表t1中的行数，每100条显示一次，缓存区为500](#查询表t1中的行数每100条显示一次缓存区为500)
    - [删除表或数据](#删除表或数据)
        - [删除行中的某个列值](#删除行中的某个列值)
            - [语法：](#语法-3)
            - [删除表t1，rowkey001中的f1:col1的数据](#删除表t1rowkey001中的f1col1的数据)
        - [删除行](#删除行)
            - [语法：](#语法-4)
            - [删除表t1，rowk001的数据](#删除表t1rowk001的数据)
        - [删除表中的所有数据](#删除表中的所有数据)
            - [语法：](#语法-5)
            - [删除表t1的所有数据](#删除表t1的所有数据)
        - [删除表](#删除表)
            - [语法：](#语法-6)
            - [删除表t1](#删除表t1)
    - [查看表](#查看表)
        ...
        - [修改表结构](#修改表结构)
            - [语法：](#语法-8)
            - [修改表test1的cf的TTL为180天](#修改表test1的cf的ttl为180天)
        - [表重命名](#表重命名)
